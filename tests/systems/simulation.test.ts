...
function inputs(x: number, y: number): Map<string, InputState> {
  return new Map<string, InputState>([[PLAYER_ID, { move: { x, y } }]]);
}

describe("Simulation", () => {
  it("rechnet in einer Sekunde Spielzeit genau so viele Ticks wie eingestellt", () => {
    const simulation = new Simulation([PLAYER_ID]);
  ...
  it("liefert unabhaengig von der Bildrate denselben Zustand", () => {
    ...
    const fast = new Simulation([PLAYER_ID]);
    const slow = new Simulation([PLAYER_ID]);
  ...
  it("interpoliert die gezeichnete Position zwischen zwei Ticks", () => {
    ...
    const atTick = simulation.renderPosition(PLAYER_ID);
    ...
    expect(simulation.alpha).toBeCloseTo(0.5, 6);
    const halfway = simulation.renderPosition(PLAYER_ID);
