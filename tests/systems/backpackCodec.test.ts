...
describe("Rucksack-Codec", () => {
  it("bringt Lage, Drehung und Starter-Merkmal unversehrt zurueck", () => {
    const items: PackedItem[] = [
      { def: 10, x: 0, y: 0, rotated: false, starter: true },
      { def: 5, x: 2, y: 1, rotated: true, starter: true },
      { def: 3, x: 4, y: 3, rotated: true, starter: false },
      { def: 0, x: 7, y: 5, rotated: false, starter: false },
    ];
  ...
  it("liest das alte Format: 1 heisst gedreht, kein Starter", () => {
    ...
    expect(unflattenPacked([4, 1, 2, 1])).toEqual([
      { def: 4, x: 1, y: 2, rotated: true, starter: false },
    ]);
  });
